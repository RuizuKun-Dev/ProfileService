...
function ProfileStore:LoadProfileAsync(profile_key, not_released_handler, _use_mock)
...
function ProfileStore:GlobalUpdateProfileAsync(profile_key, update_handler, _use_mock)
...
function ProfileStore:ProfileVersionQuery(profile_key, sort_direction, min_date, max_date, _use_mock)
	if type(profile_key) ~= "string" or string.len(profile_key) == 0 then
		error("[ProfileService]: Invalid profile_key")
	end
	if ProfileService.ServiceLocked == true then
