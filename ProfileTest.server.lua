...
local SETTINGS = {
	...
	ProfileTemplate2 = { Value = 0 },
}
local ProfileTest = { TEST_MOCK = false }
...
task.spawn(function()
	...
	if live_mode == true then
		...
		local query = GameProfileStore1:ProfileVersionQuery(
			...
			nil,
			DateTime.fromUnixTimestamp(os.time() + 10000)
		)
		local query_results = {}
		...
		for i = 1, 6 do
			...
			task.spawn(function()
				...
				if result ~= nil then
					if i == 6 then
						error(
							[[6 entries found when there should be only 5 (might be an order of query returns problem)]]
						)
					end
	...
	do
		...
		local is_passed = profile.Data.Coins == 69
			and metadata.Playtime == 123456
			and table.find(user_ids, 2312310) ~= nil
			and table.find(user_ids, 50) ~= nil
			and #profile.GlobalUpdates:GetActiveUpdates() == 0
	...
	do
		...
		local is_passed = is_color_good == true
			and metadata.Dedication == "Veteran"
			and table.find(user_ids, 2312310) ~= nil
			and table.find(user_ids, 420) ~= nil
			and table.find(user_ids, 50) == nil
	...
	GameProfileStore1:GlobalUpdateProfileAsync("Profile3", function(global_updates)
		...
		for _, update_data in pairs(global_updates:GetActiveUpdates()) do
			...
			elseif update_data[2].UpdateTag == "Test4" then
				global_updates:ChangeActiveUpdate(update_data[1], {
					UpdateTag = "Test1000000",
				})
			end
		end
	end)
	...
	pcall(function()
		updates_good = (profile_3_updates[1][2].UpdateTag == "Test2")
			and (profile_3_updates[2][2].UpdateTag == "Test3")
			and (profile_3_updates[3][2].UpdateTag == "Test1000000")
	end)
	...
	TestPass(
		"ProfileService test #4 - part 2",
		profile4.MetaData.MetaTagsLatest.LoadedFirstTime == true and updates_good
	)
	profile4:SetMetaTag("LoadedFirstTime", false)
	TestPass(
		"ProfileService test #4 - part 3",
		profile4.MetaData.MetaTagsLatest.LoadedFirstTime == true and profile4.MetaData.MetaTags.LoadedFirstTime == false
	)
	...
	TestUpdateAsync(RandomProfileStoreKey1, RandomProfileStoreScope1, "Profile7", function()
		return {
			"ThisAintRight",
		}
	end)
	...
	TestUpdateAsync(RandomProfileStoreKey1, RandomProfileStoreScope1, "Profile8", function()
		return {
			Data = {},
			MetaData = {
				ProfileCreateTime = 0,
	...
	TestUpdateAsync(RandomProfileStoreKey1, RandomProfileStoreScope1, "Profile9", function()
		return {
			Data = {},
			MetaData = {
				ProfileCreateTime = 0,
	...
	TestUpdateAsync(RandomProfileStoreKey1, RandomProfileStoreScope1, "Profile10", function()
		return {
			Data = {},
			MetaData = {
				ProfileCreateTime = 0,
	...
	profile11:Reconcile()
	TestPass(
		"ProfileService test #11",
		profile11.Data.Counter == 0 and profile11.Data.Array == false and type(profile11.Data.Dictionary) == "table"
	)
end)
