...
do
    ...
    if did_yield == true then
        error(
[[[ProfileTest]: ProfileService ModuleScript should not yield when required!]])
    end
end
...
local function TestUpdateAsync(
    store_name,
    store_scope,
    key,
    transform_function
...
task.spawn(function()
    ...
    if live_mode == true then
        ...
        local query = GameProfileStore1:ProfileVersionQuery('VersioningTest', Enum.SortDirection.Descending, nil, DateTime.fromUnixTimestamp(os.time() + 10000))
        local query_results = {}
...
[[6 entries found when there should be only 5 (might be an order of query returns problem)]])
    ...
    do
        ...
        GameProfileStore1:GlobalUpdateProfileAsync('PayloadTest', function(
            global_updates
        )
    ...
    profile2.GlobalUpdates:ListenToNewActiveUpdate(function(
        update_id,
        update_data
    )
    ...
    profile2.GlobalUpdates:ListenToNewLockedUpdate(function(
        update_id,
        update_data
    )
        if update_data.UpdateTag == 'Hello!' then
    ...
    GameProfileStore1:GlobalUpdateProfileAsync('Profile2', function(
        global_updates
    )
        global_updates:AddActiveUpdate({
            UpdateTag = 'Hello!',
    ...
    GameProfileStore1:GlobalUpdateProfileAsync('Profile3', function(
        global_updates
    )
        global_updates:AddActiveUpdate({
            UpdateTag = 'Test1',
    ...
    local profile10 = GameProfileStore1:LoadProfileAsync('Profile10', function(
        place_id,
        game_job_id
    )
